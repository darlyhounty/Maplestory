...
/*! ###################### Question for 39F ##############################*/
    ...
    var db_39 = [];
      ...
      $.ajax({
				...
				if($(aa).is("select")){
					...
					for(var i = 1; i<=8; i++){
						if($("input[name='q_36_1_" + i + "']:checked").length==0){
							...
							if(cur==1) $("#q_36_disp_" + i).text("1 花蘑菇");
							if(cur==2) $("#q_36_disp_" + i).text("2 蜗牛");
							if(cur==3) $("#q_36_disp_" + i).text("3 野猪");
							if(cur==4) $("#q_36_disp_" + i).text("4 绿水灵");
		...
		$(function(){
			window.onbeforeunload = function(e) { return '确定要关闭?'; };
		});
